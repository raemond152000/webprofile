...
for(let i = 0 ; i<365; i++){
  ...
  const list = [
    ...
    el.classList = list.includes(i) ? "box active" : "box"   //if list includes i make it active if not just the box (class = box active)
    boxContainer.appendChild(el)
}

window.addEventListener("scroll", ()=>{
  //console.log(window.scrollY);   // gives you the coordinates for horizontal disposition in scrolling
  let offsetY = window.scrollY
  saluteTextContainer.style.transform=`translateY(${offsetY * 0.3}px)`  //position of container saluteText increases as you scroll so  // so the effect will be it seems it does not move even when you scroll
  saluteImgContainer.style.transform=`translate(${offsetY * 0.4}px, ${offsetY * 0.7}px)`  //you can set it as translate X,Y axis
                                                                         // multiplying it to a factor will affects it speed of movement
  authorname.style.transform=`translateX(${offsetY * 0.5}px)`   
  authorjob.style.transform=`translateX(calc(100vh - ${offsetY}px))`  
  
  jobTitleContainers[0].style.backgroundPositionY = `${offsetY * 0.5}px`  //1st element of container //creating parallax effect                                                                      
  jobTitleContainers[1].style.backgroundPositionY = `${-offsetY * 0.5}px`  //2nd element of container //creating parallax effect                                                                      
